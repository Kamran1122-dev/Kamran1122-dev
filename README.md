<h1 align="center">Kamran Ameer</h1>
<h3 align="center">💻 Full Stack Developer | Data Science Enthusiast</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Kamran1122-dev&label=Profile%20views&color=0e75b6&style=flat" alt="Kamran1122-dev" />
</p>

<p align="left">
  <a href="https://github.com/ryo-ma/github-profile-trophy" target="_blank">
    <img src="https://github-profile-trophy.vercel.app/?username=Kamran1122-dev&theme=onedark&row=1&column=7" alt="Trophies" />
  </a>
</p>

---

### 📊 GitHub Stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kamran1122-dev&layout=compact&theme=radical" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Kamran1122-dev&show_icons=true&locale=en&theme=radical" alt="GitHub Stats" />
</p>
---

### 🧭 Career Objective

To contribute my skills and education in computer programming and networking to an organization where they can be effectively utilized.  
My goal is to work in a challenging environment that fosters both institutional growth and personal skill development.

---

### 🎓 Education

- **Master’s in Data Science** – *University of Gloucestershire* (Sep 2024)  
- **MCS – Computer Science** – *University of Management & Technology, Lahore* (Feb 2021) – GPA: 2.76  
- **BSc – Computer Science (2 Years)** – *University of Punjab* (2017) – 2nd Division  

**Certifications:**
- 🛡️ SIA Door Supervisor Security Badge  
- ⛑️ Level 3 Emergency First Aid Certificate  

---

### 💼 Experience

- **Full Stack Developer – IT Prism (Private) Ltd, Islamabad**  
  *Feb 2021 – Present (Remote)*  
  - Developed enterprise-level web applications for international clients.  
  - Integrated UX/UI with backend using modern web technologies.  
  - Managed deployments, GitHub repositories, and project tracking boards.  
  - Delivered technical presentations to international clients.  

- **Inside Sales Executive – Punjab Cash and Carry, Islamabad**  
  *Feb 2021 – Mar 2023*  

- **Freelancer (Full Time)**  
  *2013 – Present*  
  - Worked across multiple web technologies and delivered client-focused solutions globally.  

---

### 🧠 Core Strengths

- Microsoft Office Suite (Excel, Word, Access, Databases)  
- SQL Server & C++  
- PHP (OOP), JavaScript, HTML, CSS  
- GitHub, Bitbucket, and version control systems  
- Database design and software installation/configuration  
- Network setup: Routers, Switches, NAS devices  
- Operating Systems: Windows, Linux, macOS  
- Team management, error handling, and project supervision  

---

### 💬 Languages

- **English:** Fluent (Read, Write, Speak)  
- **Urdu:** Native  

---

### 🌐 Connect with Me

<p align="left">
  <a href="mailto:kamranameer.smiles@gmail.com" target="_blank">
    <img src="https://cdn.jsdelivr.net/npm/simple-icons@v4/icons/gmail.svg" alt="Email" height="30" width="40"/>
  </a>
</p>

📫 **Email:** kamranameer.smiles@gmail.com  
📍 **Address:** 54 Hawthrone St, Deane, Bolton BL3 5RG  
📞 **Phone:** 07466314801  

---

### 🛠️ Tools & Technologies

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/windows8/windows8-original.svg" width="40" height="40"/>
</p>

---

### ⚡ Fun Fact

> “Technology is best when it brings people together — I strive to build solutions that connect ideas, people, and innovation.”

---

> _“Code is like humor. When you have to explain it, it’s bad.” – Cory House_
